...
response envelope is identical regardless of which provider served the
...
# Providers occasionally invent enum values (e.g. type "shape"). Coerce them to
# "other" up front so one bad block does not discard the whole observation /
# element list during validation.
_OBS_TYPES = set(ObservationType.__args__)  # type: ignore[attr-defined]
_ELEM_TYPES = set(ElementType.__args__)  # type: ignore[attr-defined]


def _sanitize_enums(raw: dict) -> dict:
    raw = dict(raw)  # never mutate the caller's dict
    for key, allowed in (("observations", _OBS_TYPES), ("elements", _ELEM_TYPES)):
        items = raw.get(key)
