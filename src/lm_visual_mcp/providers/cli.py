"""Shared machinery for CLI-based providers (AGY, Codex).

Extends :class:`Provider`, so every CLI provider also carries its own
...
class CliProvider(Provider):
    ...
    def parse_result(self, result: SubprocessResult, request: ImageRequest) -> ProviderResult:
        try:
    ...
    def _log_stderr(self, result: SubprocessResult) -> None:
        ...
        logging.getLogger("lm_visual_mcp.vision.providers").debug(
            "%s stderr (rc=%s): %s", self.name, result.returncode, result.stderr[:2000]
        )

    @staticmethod
    def _media_instructions(request: ImageRequest, base_dir: Optional[Path] = None) -> str:
