"""Vision module domain types (image only).

These types are the only currency exchanged between the router and providers.
Providers never see MCP tool names; the MCP layer never sees provider details.
This version supports image recognition only - video input is intentionally
